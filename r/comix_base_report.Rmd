...
timetable <- part %>% group_by(wave) %>% summarize(min=min(date), max=max(date))
names(timetable) <- c("Wave","Start date","End date")
##I want to transpose this so as we get more waves, the table grows horizontally not down...
#timetable <- as_tibble(t(timetable), .rows = NULL)
kable(timetable) %>% kable_styling(full_width = F, position = "left")
...
contacts <- left_join(contacts, part_ages, by = c("country", "panel", "wave", "part_id"))
...
trunc_mean_age <- trunc_contacts %>% 
  ...
  group_by(wave, part_age_group) %>%
  summarise("Mean contacts" = mean(n)) %>%
  mutate(Contacts = "Truncated to 50 contacts per participant") %>% 
  rename("Participant Age" = part_age_group) 
...
ggplot(contact_means_ages, aes(x = factor(Wave), y = `Mean contacts`, fill = `Participant Age`)) +
  geom_col(position = "dodge") +
  xlab("Wave") +
  facet_grid(vars(Contacts)) +
  theme_bw() +
