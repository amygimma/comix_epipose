...
The CoMix survey in `r country_name_` has had `r length(unique(part$wave))` waves of data collection covering from the period `r format(min(part$survey_date), "%d of %B %Y")` to `r format(max(part$surey_date), "%d of %B %Y")`. Figure 1 shows survey response by date.
