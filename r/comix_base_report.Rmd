...
kable(cnt_summary_table) %>% kable_styling(full_width = F, position = "left")
```
...
trunc_mean <- trunc_contacts %>% 
  ...
  group_by(wave) %>%
  summarise("Mean contacts" = mean(n)) %>%
   mutate(Contacts = "Truncated 50 / participant", "Participant Age" = "All") 
...
contact_means_table <- contact_means %>%
  pivot_wider(id_cols = c("Contacts", "Participant Age"), names_from = "Wave", values_from = "Mean contacts", names_prefix = "Wave ")
