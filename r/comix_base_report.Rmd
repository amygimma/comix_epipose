...
timetable <- part %>% group_by(wave) %>% 
  summarize(min_date = format(min(date), "%d %b %y"), 
            max_date = format(max(date), "%d %b %y"), 
            participants = n()) %>% 
  ungroup()
...
kable(adj_mean_contacts_table_all, digits = 2) %>% kable_styling()
```
...
kable(adj_mean_contacts_table_adult, digits = 2) %>% kable_styling()
```
