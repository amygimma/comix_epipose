...
The CoMix survey in `r country_name_` has had `r length(unique(part$wave))` waves of data collection covering the period between `r format(min(part$survey_date), "%d %B %Y")` and `r format(max(part$survey_date), "%d %B %Y")`from adults. Figure 1 shows survey response by date
...
child_sch_attendance <- part %>% filter(panel=="C" & !is.na(part_age_group)) %>% 
...
setting_all_sch <- ggplot(
  contacts %>% filter(panel == "C" & went_to_school!="NA"), 
...
wave_num_A <- nrow(distinct((contacts %>% filter(panel=="A") %>% select(wave_id))))
grid.arrange(setting_all, setting_all_sch, ncol=2, widths=c(wave_num_A,5))

setting_individual <- ggplot(
  contacts %>% filter(panel == "A" & cnt_mass == "individual"), 
  aes(y = as.character(wave_id), fill = factor(cnt_setting))) + 
  geom_bar(position="fill", stat="count") +
  ylab("") +
  xlab("Proportion") +
...
setting_individual_sch <- ggplot(
  contacts %>% filter(panel == "C" & went_to_school!="NA" & cnt_mass == "individual"),   
  aes(y = wave_sch_id, fill = factor(cnt_setting))) + 
  geom_bar(position="fill", stat="count") +
  ggtitle("(Did child attend school yesterday?)") +
  ylab("") +
...
cnt_summary_setting_sch <- contacts %>%
  filter(panel == "C" & went_to_school!="NA") %>% 
...
total_time_all <- ggplot(
  contacts %>% filter(cnt_mass == "individual") %>% filter(panel == "A"), 
...
total_time_non_household <- ggplot(
  contacts %>% filter(cnt_mass == "individual" & cnt_household == 0) %>% filter(panel=="A"), 
  aes(y = as.character(wave_id), fill = factor(cnt_total_time))) + 
  geom_bar(position="fill", stat="count") +
  ylab("") +
  xlab("Proportion") +
